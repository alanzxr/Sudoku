    ...
    def bs(self):
        ...
        for k in range(self.q[self.v - 1][2] + 1, 10):
            if self.S[self.q[self.v - 1][0]][self.q[self.v - 1][1]][k] == 1:
                ...
                self.q[self.v - 1][2] = k
                self.S[self.q[self.v - 1][0]][self.q[self.v - 1][1]][self.A[self.q[self.v - 1][0]][self.q[self.v - 1][1]]] = 0
                for kk in range(9):
                    self.S[self.q[self.v - 1][0]][kk][self.A[self.q[self.v - 1][0]][self.q[self.v - 1][1]]] = 0
                    self.S[kk][self.q[self.v - 1][1]][self.A[self.q[self.v - 1][0]][self.q[self.v - 1][1]]] = 0
                for kk in range(int(self.q[self.v - 1][0] / 3) * 3, int(self.q[self.v - 1][0] / 3) * 3 + 3):
                    for ll in range(int(self.q[self.v - 1][1] / 3) * 3, int(self.q[self.v - 1][1] / 3) * 3 + 3):
                        self.S[kk][ll][self.A[self.q[self.v - 1][0]][self.q[self.v - 1][1]]] = 0
                return
        self.v -= 1
        if self.v < 1:
            print('END')
